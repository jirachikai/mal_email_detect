...
def create_vocabulary(vocabulary_path, data_path, tokenizer=None):
  ...
  Data file is assumed to contain one email per line and tag after it seperated by ",". 
  Each email is tokenized. We write it to vocabulary_path in a one-token-per-line format,
  so that later token in the first line gets id=0, second line gets id=1, and so on.
...
def email_to_token_ids(email, vocabulary,
...
def prepare_data(data_dir, from_train_path, to_train_path, from_dev_path, to_dev_path, from_vocabulary_size,
  ...
  """Preapre all necessary files that are required for the training.
    ...
    Args:
      ...
      train_path: path to the file that includes training data
      test_path: path to the file that includes testing data.
